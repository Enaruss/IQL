...
import iql.engine.utils.PropsUtils.confMap
import java.net.URI
...
object PropsUtils {
  ...
  val confMap: Map[String, String] = getPropertiesFromLocalFile(
    "iql.properties"
  )
  ...
  def getOrNull(k: String): String = {
    if (confMap.contains(k)) confMap(k) else null
  }
  ...
  def getPropertiesFromHDFSFile(path: String): Map[String, String] = {
    ...
    try {
      val fs =
        FileSystem.get(URI.create(confMap("hdfs.url")), new Configuration())
  ...
  def getPropertiesFromLocalFile(fileName: String): Map[String, String] = {
    try {
      val inputStream =
        PropsUtils.getClass.getClassLoader.getResourceAsStream(fileName)
      val properties = new Properties()
      ...
      case e: Exception =>
        throw new Exception(
          s"Failed when loading iql properties from $fileName",
          e
        )
    }
  }
